...
class YtDlpBackend:
    ...
    def _build_ydl_opts(self, output_template: str) -> dict:
        ...
        if hasattr(config, "ffmpeg_location") and config.ffmpeg_location:
    ...
    def download(self, url: str, line_number: int = 0, custom_name: str | None = None) -> DownloadResult:
        ...
        if HAS_CURL_CFFI:
            ...
            probe_opts["impersonate"] = ImpersonateTarget(client="chrome")
        if hasattr(config, "ffmpeg_location") and config.ffmpeg_location:
            probe_opts["ffmpeg_location"] = config.ffmpeg_location
        if config.cookies_from_browser:
